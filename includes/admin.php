...
if (!function_exists('mrwcmc_sanitize_settings')) {
    function mrwcmc_sanitize_settings($input) : array {
        ...
        // Supported currencies (restrict to valid WooCommerce currencies)
        ...
        $output['supported_currencies'] = array_values(array_intersect($supported, $valid));
        if (empty($output['supported_currencies'])) $output['supported_currencies'] = [$output['base_currency']];

        // Country → Currency map from textarea
        $map_raw = isset($input['country_currency_map_raw']) ? wp_unslash($input['country_currency_map_raw']) : '';
        $map = mrwcmc_parse_key_pairs($map_raw);
        if (!isset($map['*'])) { $map['*'] = $output['base_currency']; }
        $output['country_currency_map'] = $map;
