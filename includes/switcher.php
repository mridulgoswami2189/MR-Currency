    ...
    function mrwcmc_switcher_render(array $args = []) : string {
        ...
        if ($type === 'links') {
            foreach ($supported as $c) {
                $url = add_query_arg('mrwcmc', $c);
                $cls = $c === $current ? ' style="font-weight:600;text-decoration:underline;"' : '';
                $out .= '<a href="' . esc_url($url) . '"' . $cls . '>' . esc_html($c) . '</a> ';
            }
        } else {
            // simple dropdown that navigates with ?mrwcmc= (param-guard will strip it)
            ...
            $out .= '</select>';
            $out .= '</form>';
            $out .= '<script>(function(){var s=document.getElementById("mrwcmc_currency_select"); if(!s) return; s.addEventListener("change",function(){var cur=this.value; if(!cur) return; var u=new URL(window.location.href); u.searchParams.set("mrwcmc",cur); window.location.assign(u.toString());});})();</script>';
        }
...
if (!function_exists('mrwcmc_switcher_action')) {
    function mrwcmc_switcher_action($args = []) { echo mrwcmc_switcher_render(is_array($args)?$args:[]); }
